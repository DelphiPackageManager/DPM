...
var
  ...
  //Every consumer compiles this project with msbuild at install time. A package project has no
  ...
  begin
    if projectPath = '' then
      exit;
    onDisk := TPath.Combine(FEffectiveBasePath, NormalisedProjectPath(projectPath));
    blocking := TCollections.CreateList<string>;
    advisory := TCollections.CreateList<string>;
    //A project not on disk yet - generated by an upstream step, the case the wildcard fallback in
    //CheckProjectIncluded exists for - cannot be inspected. The install time scan is the backstop.
    if not TBuildHookValidator.TryScanFile(onDisk, blocking, advisory) then
      exit;
...
begin
  ...
  begin
    ...
    CheckProjectHooks('Build entry', buildEntry.Project);
  end;
  ...
  begin
    ...
    //A prebuilt entry points at a .bpl rather than a dproj - there is no msbuild file to inspect.
    if not designEntry.IsPrebuilt then
      CheckProjectHooks('Design entry', designEntry.Project);
  end;
end;
