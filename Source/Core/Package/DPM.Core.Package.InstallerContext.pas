...
var
  projectRes : IDictionary<TDPMPlatform, IList<IResolution>>;
  resList : IList<IResolution>;
  ...
  begin
    ...
    begin
      resolution := resList.FirstOrDefault(
        function(const value : IResolution) : boolean
        begin
             result := SameText(value.Package.Id, packageId);
        end);
